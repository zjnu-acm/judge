...
public class AdminProblemListController {
    ...
    public String problemlist(HttpServletRequest request, @PageableDefault(100) Pageable pageable) {
        UserDetailService.requireAdminLoginned(request);

        long total = problemMapper.count();
        Page<Problem> page = new PageImpl<>(problemMapper.findAll(pageable), pageable, total);
