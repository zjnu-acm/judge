 ...
 */
@SuppressWarnings({"FinalMethod", "FieldNameHidesFieldInSuperclass"})
public class CKFinderTag extends TagSupport {
    ...
    private String buildUrl() {
        ...
        if (this.selectFunction != null && !this.selectFunction.isEmpty()) {
            qs = qs + "?action=js&amp;func=" + this.selectFunction;
        }

        if (this.selectFunctionData != null && !this.selectFunctionData.isEmpty()) {
            qs += !isNullOrEmpty(qs) ? "&amp;" : "?";
            try {
        ...
        if (this.disableThumbnailSelection) {
            ...
            qs += !isNullOrEmpty(qs) ? "&amp;" : "?";
            qs = qs + "thumbFunc=" + (!isNullOrEmpty(this.selectThumbnailFunction)
