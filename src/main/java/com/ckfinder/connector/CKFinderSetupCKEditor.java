...
public class CKFinderSetupCKEditor extends TagSupport {
    ...
    public int doStartTag() throws JspException {
        ...
        if (flashType == null || flashType.isEmpty()) {
            flashType = DEFAULT_FLASH_TYPE;
        }
        Map<String, Map<String, String>> attr = new HashMap<>(1);
    ...
    private String buildUploadUrl(final String type) {
        if (pageContext.getRequest() instanceof HttpServletRequest) {
            return ((HttpServletRequest) pageContext.getRequest()).getContextPath() + CKFINDER_UPLOAD_URL + type;
        } else {
